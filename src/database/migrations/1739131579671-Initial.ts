...
export class Initial1739131579671 implements MigrationInterface {
    name = 'Initial1739131579671'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "sto_info_app"."account" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "handle" character varying(255) NOT NULL, "username" character varying(255), "email" character varying(255), "notes" text, "accountCreatedDate" TIMESTAMP NOT NULL, "publiclyVisible" boolean NOT NULL DEFAULT false, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP, "platformId" uuid, "launcherId" uuid, "userId" uuid, CONSTRAINT "PK_54115ee388cdb6d86bb4bf5b2ea" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "sto_info_app"."user" ("id" uuid NOT NULL, "email" character varying(255) NOT NULL, "username" character varying(255), "firstName" character varying(255), "lastName" character varying(255), "profilePicture" character varying, "password" character varying(255) NOT NULL, "emailVerified" boolean NOT NULL DEFAULT false, "emailVerificationToken" character varying, "emailVerificationTokenExpiry" TIMESTAMP, "lastLogin" TIMESTAMP, "lastPasswordReset" TIMESTAMP, "passwordResetToken" character varying, "passwordResetTokenExpiry" TIMESTAMP, "isAccountDisabled" boolean NOT NULL DEFAULT false, "provider" character varying, "providerId" character varying, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP, CONSTRAINT "UQ_e12875dfb3b1d92d7d7c5377e22" UNIQUE ("email"), CONSTRAINT "UQ_78a916df40e02a9deb1c4b75edb" UNIQUE ("username"), CONSTRAINT "UQ_f4ca2c1e7c96ae6e8a7cca9df80" UNIQUE ("email", "username"), CONSTRAINT "PK_cace4a159ff9f2512dd42373760" PRIMARY KEY ("id"))`);
