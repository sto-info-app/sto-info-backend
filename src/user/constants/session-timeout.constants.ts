 ...
 * the `CHK_user_preference_session_timeout` constraint added by
 * `CreateUserPreference` rejects anything else at the database, and the client
 * offers the matching labels. Changing it means changing all three.
 */
 ...
 *
 * A user who has never chosen falls back to the deployment's configured
 ...
 *
 * @param selected - The window stored in the user's preferences, if any.
